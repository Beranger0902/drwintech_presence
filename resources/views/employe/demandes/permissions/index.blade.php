...
<head>
    ...
    <title>Demandes de permission</title>
            ...
            <nav class="menu">
                ...
                <div class="menu-dropdown open" id="menuDropdownDemandes">
                    ...
                    <div class="menu-submenu">
                        ...
                        <a href="{{ route('employe.demandes.permissions.index') }}" class="active">
        ...
        <main class="content">
            <div class="content-page-header">
                ...
                <div class="page-breadcrumb-row">
                    ...
                    <span>/</span>
                    <a href="{{ route('employe.demandes.permissions.index') }}" class="active-breadcrumb">Permission</a>
                </div>
            </div>
            ...
            <div class="table-card">
                <div class="table-wrapper">
                    <table>
                        ...
                        <tbody>
                            @forelse ($demandesPermission as $demande)
                                <tr>
                                    <td>{{ optional($demande->permission?->date_permission)->format('d/m/Y') ?? '-' }}</td>
                                    <td>
                                        {{ $demande->permission?->heure_debut ?? '-' }}
                                        -
                                        {{ $demande->permission?->heure_fin ?? '-' }}
                                    </td>
                                    <td>
                                        @if ($demande->permission)
                                            {{ $demande->permission->duree }} min
                                        @else
                                            -
                                        @endif
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" style="text-align:center;">Aucune demande de permission.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="pagination-wrapper">
                    {{ $demandesPermission->links() }}
                </div>
            </div>
        </main>
    </div>

    <div class="modal-overlay {{ $errors->any() ? 'show' : '' }}" id="permissionModal">
        <div class="modal-box">
            <div class="modal-header">
                <h2>Nouvelle demande de permission</h2>
                <button type="button" class="close-modal-btn" id="closePermissionModal">&times;</button>
            </div>

            <div class="modal-body">
                <form method="POST" action="{{ route('employe.demandes.permissions.store') }}">
                    @csrf

                    <div class="form-group">
                        <label for="date_permission">Date</label>
                        <input type="date" name="date_permission" id="date_permission" value="{{ old('date_permission') }}" required>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
            ...
            const openPermissionModal = document.getElementById('openPermissionModal');
