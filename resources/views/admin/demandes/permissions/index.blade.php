...
<head>
    ...
    <style>
        * {
        ...
        .status-pending { background: #e7b11d; color: #3a2d00; }
        .status-approved { background: #41b66a; }
        .status-refused { background: #2f7de1; }
...
<body>
            ...
            <nav class="menu">
                ...
                <div class="menu-dropdown open" id="menuDropdownDemandes">
                    ...
                    <div class="menu-submenu">
                        ...
                        <a href="{{ route('admin.demandes.permissions.index') }}" class="active">
        ...
        <main class="content">
            <div class="content-header">
                ...
                <div class="breadcrumb">
                    ...
                    &nbsp; / &nbsp;
                    <a href="{{ route('admin.demandes.permissions.index') }}">Permission</a>
                </div>
            </div>
            ...
            <div class="toolbar-card">
                ...
                <div class="toolbar-right">
                    <form method="GET" action="{{ route('admin.demandes.permissions.index') }}" style="display:flex; gap:12px; align-items:center; flex-wrap:wrap;">
            ...
            <div class="stats-grid">
                ...
                <div class="summary-card summary-card-yellow">
                    ...
                    <div>
                        ...
                        <div class="summary-value">{{ $permissionsEnAttente }}</div>
                    </div>
                </div>
                ...
                <div class="summary-card summary-card-green">
                    ...
                    <div>
                        ...
                        <div class="summary-value">{{ $permissionsApprouvees }}</div>
                    </div>
                </div>
                ...
                <div class="summary-card summary-card-blue">
                    ...
                    <div>
                        ...
                        <div class="summary-value">{{ $permissionsRefusees }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="table-card">
                <div class="table-wrap">
                    <table>
                        ...
                        <tbody>
                            @forelse($permissions as $permissionDemande)
                                @php
                                    $statutClass = match($permissionDemande->statut) {
                                        'approuve' => 'status-approved',
                                        'refuse' => 'status-refused',
                                        default => 'status-pending',
                                    };

                                    $statutLabel = match($permissionDemande->statut) {
                                        'approuve' => 'Approuvé',
                                        'refuse' => 'Refusé',
                                        default => 'En attente',
                                    };
                                ...
                                <tr>
                                    <td>{{ $permissionDemande->employe?->nom }} {{ $permissionDemande->employe?->prenom }}</td>
                                    <td>
                                        {{ $permissionDemande->permission?->date_permission
                                            ? \Carbon\Carbon::parse($permissionDemande->permission->date_permission)->format('d/m/Y')
                                            : '--/--/----' }}
                                    </td>
                                    <td>{{ $permissionDemande->permission?->heure_debut ?? '--:--' }}</td>
                                    <td>{{ $permissionDemande->permission?->heure_fin ?? '--:--' }}</td>
                                    <td>{{ $permissionDemande->observation ?? 'Aucun motif' }}</td>
                                    <td>
                                        <span class="status-badge {{ $statutClass }}">{{ $statutLabel }}</span>
                                    </td>
                                    <td>
                                        <div class="actions-cell">
                                            <a href="{{ route('admin.demandes.permissions.index', array_merge(request()->query(), ['view' => $permissionDemande->id])) }}" class="action-btn action-view">
                                                <svg viewBox="0 0 24 24">
                                            ...
                                            <form method="POST" action="{{ route('admin.demandes.permissions.approve', $permissionDemande->id) }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="action-btn action-approve" {{ $permissionDemande->statut !== 'en_attente' ? 'disabled' : '' }}>
                                                    <svg viewBox="0 0 24 24">
                                                        <path d="M20 6L9 17L4 12"></path>
                                                    </svg>
                                                </button>
                                            ...
                                            <form method="POST" action="{{ route('admin.demandes.permissions.refuse', $permissionDemande->id) }}">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="action-btn action-refuse" {{ $permissionDemande->statut !== 'en_attente' ? 'disabled' : '' }}>
                                                    <svg viewBox="0 0 24 24">
                                                        <path d="M18 6L6 18"></path>
                                                        <path d="M6 6L18 18"></path>
                                                    </svg>
                                            ...
                                            @if ($permissionDemande->employe->refusals_count >= 3)
                                                <form method="POST" action="{{ route('admin.demandes.permissions.debloquer', $permissionDemande->id) }}">
                                                    @csrf
                                                    @method('PATCH')
                            ...
                            @empty
                                <tr>
                                    <td colspan="7">Aucune demande de permission trouvée.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    @if($selectedPermission && $openModal === 'view')
        <div class="modal-overlay show" id="viewModal">
            <div class="modal-box">
                <div class="modal-header">
                    <h2>Détail Permission</h2>
                    <a href="{{ route('admin.demandes.permissions.index', request()->except(['view'])) }}" class="close-modal-btn" style="text-decoration:none;">&times;</a>
                </div>
                ...
                <div class="modal-body">
                    <div class="detail-list">
                        <div class="detail-row">
                            ...
                            <div class="detail-value">{{ $selectedPermission->employe?->nom }} {{ $selectedPermission->employe?->prenom }}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Email</div>
                            <div class="detail-value">{{ $selectedPermission->employe?->user?->email }}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Date permission</div>
                            <div class="detail-value">
                                {{ $selectedPermission->permission?->date_permission
                                    ? \Carbon\Carbon::parse($selectedPermission->permission->date_permission)->format('d/m/Y')
                                    : '--/--/----' }}
                            </div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Heure début</div>
                            <div class="detail-value">{{ $selectedPermission->permission?->heure_debut ?? '--:--' }}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Heure fin</div>
                            <div class="detail-value">{{ $selectedPermission->permission?->heure_fin ?? '--:--' }}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Motif</div>
                            <div class="detail-value">{{ $selectedPermission->observation ?? 'Aucun motif' }}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Statut</div>
                            <div class="detail-value">
                                {{ $selectedPermission->statut === 'approuve' ? 'Approuvé' : ($selectedPermission->statut === 'refuse' ? 'Refusé' : 'En attente') }}
                            </div>
                        </div>
                    </div>

                    <div class="modal-actions">
                        <a href="{{ route('admin.demandes.permissions.index', request()->except(['view'])) }}" class="btn-secondary">Fermer</a>
                    </div>
                </div>
            </div>
        </div>
